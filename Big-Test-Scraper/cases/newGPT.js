...
fs.readFile('cases.json', 'utf8', (err, data) => {
  ...
  jsonData.forEach((caseData) => {
    const fileName = `case-${caseData}.jpg`; // Generate a unique filename based on the case name
    // const fileName = caseData.name; // Generate a unique filename based on the case name
    const filePath = folderPath + fileName; // Generate the full file path
    // const filePath = fileName; // Generate the full file path
